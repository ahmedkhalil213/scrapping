...
class ParserLocalCh:
    ...
    """
    def __init__(self):
        self.base_url = "https://www.local.ch/fr/q/lausanne/annuaire-telefonique"
    ...
    def parse(self, index_page=None):
        if index_page is None:
            ...
            link_to_use = f"{self.base_url}?page={index_page}"
    ...
    def parse_company(self, link_company):
        ...
        comma_index =adresse.find(',')
        lausanne_index =adresse.find('Lausanne')
        number_between_comma_and_lausanne = adresse[comma_index + 2:lausanne_index].strip()
        if len(liste_numbers)>0:
            ...
            self.data[company_name]["code postale"] = number_between_comma_and_lausanne
...
def save_json_to_excel(json_path, path):
    ...
    # Add headers
    headers = ['Nom Particulier', 'numéro téléphone', 'Adresse', 'Code Postal']
    ...
    for row_num, (company_name, company_data) in enumerate(data.items(), start=2):
        ws.cell(row=row_num, column=1, value=company_name)  # 'Nom Particulier'
        ws.cell(row=row_num, column=2, value=", ".join(company_data.get('phone_numbers', [])))
        ws.cell(row=row_num, column=3, value=company_data.get('adresse'))
        ws.cell(row=row_num, column=4, value=company_data.get('code postale'))
...
# save_json_to_excel("data.json", "particulier-lausanne.xlsx")